...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function C(vertexShader, fragmentShaderSource) {
    ...
    C.prototype.setKeywords = function (keywords) {
      let hash = 0;
    ...
    function P(vertexShader, fragmentShader) {
      this.uniforms = {};
    ...
    const blurVertexShader = compileShader(
      gl.VERTEX_SHADER,
      "precision highp float; attribute vec2 aPosition; varying vec2 vUv; varying vec2 vL; varying vec2 vR; uniform vec2 texelSize; void main () {     vUv = aPosition * 0.5 + 0.5;     float offset = 1.33333333;     vL = vUv - texelSize * offset;     vR = vUv + texelSize * offset;     gl_Position = vec4(aPosition, 0.0, 1.0); }"
    );
    const blurShader = compileShader(
      gl.FRAGMENT_SHADER,
      "precision mediump float; precision mediump sampler2D; varying vec2 vUv; varying vec2 vL; varying vec2 vR; uniform sampler2D uTexture; void main () {     vec4 sum = texture2D(uTexture, vUv) * 0.29411764;     sum += texture2D(uTexture, vL) * 0.35294117;     sum += texture2D(uTexture, vR) * 0.35294117;     gl_FragColor = sum; }"
    );
    ...
    const copyProgram = new P(baseVertexShader, copyShader);
    const clearProgram = new P(baseVertexShader, clearShader);
    const colorProgram = new P(baseVertexShader, colorShader);
    const checkerboardProgram = new P(baseVertexShader, checkerboardShader);
    const splatProgram = new P(baseVertexShader, splatShader);
    const advectionProgram = new P(baseVertexShader, advectionShader);
    const divergenceProgram = new P(baseVertexShader, divergenceShader);
    ...
    function step(dt) {
      ...
      let velocityTexId = velocity.read.attach(0);
